			...
			<div class="col-sm-12">
				<div class="panel panel-border panel-info">
					...
					<div class="panel-body">
						<form action="{{ route('user.store', ['user_id' => $selected_info->id ?? null]) }}" method="POST"
							...
							<div class="row">
								...
								<div class="col-sm-4">
									<div class="form-group">
										...
										<input type="text" name="mobile" id="mobile" value="{{ old('mobile') }}" placeholder="Mobile Number"
								...
								<div class="col-sm-4">
									<div class="form-group">
										<label for="password">Confirm Password</label><small class="req"> *</small><a
											href="javascript:void(0)" onclick="showPassword()"><i class="fa fa-eye"></i></a>
										<input type="password" name="confirm_password" id="confirm_password"
											onkeyup="checkPass(); return false;" placeholder="Confirm Password..." class="form-control">
								...
								<div class="col-sm-4">
									<div class="form-group">
										...
										<input data-max-file-size="1024K" data-allowed-file-extensions="jpg png jpeg" name="photo"
								...
								<div class="col-sm-4">
									<div class="form-group pull-left m-t-22 m-l-15 ">
										<button name="add_user" type="submit" class="btn btn-primary"><i class="md md-add m-r-5"></i>Add
											User</button>
									</div>
								</div>
							</div>
								...
								<table id="example" class="table table-striped table-bordered dt-responsive nowrap">
									...
									<tbody>
										...
										<tr>
											...
											<td>{{ $value->name }} @if(logged_in_user_id() == $value->id) <span
													class="label label-default bg-light-success bg-success-color m-b-5 d-inline-block">(You)</span>
												@endif</td>
											<td>{{ $value->mobile }}-({{ $value->email }})</td>
											<td>
												<a onclick="return confirm('Are You Sure?')"
													href="{{ route('user.control',['user_id' => $value->id]) }}"
													title="{{ $value['status'] == 1 ? 'Enable' : 'Disable' }}"
													class="text-{{ $value['status'] == 1 ? 'success' : 'danger' }} btn btn-default btn-xs waves-effect tooltips"
													data-placement="top" data-toggle="tooltip"><i class="fa fa-check-circle"></i></a>

												<a onclick="return confirm('Are You Sure?')"
													href="{{ route('user.destroy',['user_id' => $value->id]) }}"
													class="text-danger btn btn-default  btn-xs  waves-effect tooltips" data-placement="top"
													data-toggle="tooltip" data-original-title="Delete"><i class="fa fa-trash"></i></a>
											</td>
										</tr>
										@endforeach
									</tbody>
...
<script>
	...
	$('#mobile').on('keyup', function () {
		var regx = /(^(\+88|0088)?(01){1}[3456789]{1}(\d){8})$/;
		if (regx.test($('#mobile').val())) {
			$(".help-block").fadeOut(100);
		} else {
			$(".help-block").fadeIn();
			$(".help-block").html("<code>Not valid mobile no.!</code>");
		}
	});
