...
use App\Rules\AgeBetween3And20;
...
class StudentController extends Controller
{
    ...
    public function studentLoginCheck (Request $request) {
        ...
        $studentInfo = Student::where('user_name', $request->username)->where('password', md5($request->password))->where('status', 1)->first();
        if ($studentInfo){
            ...
            setMessage("message", "danger", "Operation Fail");
            return redirect('/');
        }
    }
    ...
    public function myAccount(){
        ...
        $data['total_courses'] = StudentDetail::where('status', 1)->where('student_id', $studentId)->count();
    ...
    public function updateProfile(Request $request){
        ...
        if ($request->input('password')) {
            ...
            $this->validate($request, [
                ...
                'phone'       => ['required', 'regex:/^(\\+88|88)?(01){1}[3456789]{1}(\\d){8}$/'], // Regular expression pattern for 10-digit mobile number
                'age'         => ['required', 'integer', new AgeBetween3And20],
                'grade'       => 'required',
                'district'    => 'required',
                'fb_link'     => 'required',
            ]);
